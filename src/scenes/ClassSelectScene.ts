...
const CLASS_ORDER: SystemPrompt[] = ['autocomplete', 'hallucinate', 'analyze', 'jailbreak'];
...
export default class ClassSelectScene extends Phaser.Scene {
  ...
  private move(dir: number) {
    this.selected = (this.selected + dir + CLASS_ORDER.length) % CLASS_ORDER.length;
  ...
  private confirm() {
    ...
    this.time.delayedCall(450, () => this.scene.start('GameScene', { systemPrompt: CLASS_ORDER[this.selected] }));
  }
  ...
  private updateSelectionColors() {
    for (let i = 0; i < CLASS_ORDER.length; i++) {
      ...
      this.optionPrefixes[i].setColor(active ? color : '#334455');
      this.optionLabels[i].setColor(active ? color : '#445566').setAlpha(active ? 1 : 0.45);
    }
  }
  ...
  private typeDetails() {
    ...
    const vals = [
      ...
      { name: 'RTE', v: 1000 / stats.fireRate, max: maxes.rate, text: `${Math.round(1000 / stats.fireRate)}/s` },
    ];
    for (let i = 0; i < 4; i++) {
  ...
  private typeText(obj: Phaser.GameObjects.Text, text: string, speed: number): Phaser.Time.TimerEvent {
    let i = 0;
  ...
  update(_time: number, delta: number) {
    ...
    if (this.blinkTimer > 420) { this.blinkTimer = 0; this.blinkOn = !this.blinkOn; }

    this.gfx.clear();
    this.previewGfx.clear();

    this.gfx.lineStyle(1, 0x1a2a3a, 0.2);
    ...
    if (blink) {
      this.previewGfx.fillStyle(stats.color, 0.85);
      this.previewGfx.fillRect(pvX - 4, pvY - 26, 8, 52);
      this.previewGfx.fillStyle(0xffffff, 0.9);
      this.previewGfx.fillRect(pvX - 2.5, pvY - 24, 5, 48);
    }
    this.previewGfx.lineStyle(2, stats.color, 0.35 + Math.sin(this.breathPhase * 3) * 0.15);
    ...
    for (let i = 0; i < 8; i++) {
      ...
      this.previewGfx.fillStyle(stats.color, 0.3 + Math.sin(this.breathPhase * 3 + i) * 0.15);
