...
import Cursor, { type SystemPrompt, type WeaponMod, WEAPON_MOD_COLORS, WEAPON_MOD_NAMES } from '../objects/Cursor';
...
const BOSS_ORDER: BossType[] = ['contentFarm', 'blackBox', 'hallucinator', 'alignmentProblem', 'overfitEngine', 'promptInjector', 'singularity'];

const ZONE_ENEMIES: EnemyType[][] = [
  ['loremIpsum', 'watermark'],
  ['loremIpsum', 'watermark', 'clickbait'],
  ['watermark', 'clickbait', 'bias'],
  ['clickbait', 'bias', 'deepfake'],
  ['bias', 'deepfake', 'scraper'],
  ['deepfake', 'scraper', 'overfit'],
  ['loremIpsum', 'watermark', 'clickbait', 'bias', 'deepfake', 'scraper', 'overfit'],
];
...
type GameState = 'playing' | 'bossIntro' | 'boss' | 'bossDeath' | 'victory' | 'gameOver';
...
export default class GameScene extends Phaser.Scene {
  ...
  private pickups: { x: number; y: number; type: 'token' | 'health' | 'weapon'; weaponMod?: WeaponMod; tier?: number; age: number; collected: boolean }[] = [];
  ...
  private keys!: {
    ...
    e: Phaser.Input.Keyboard.Key; m: Phaser.Input.Keyboard.Key;
  ...
  create() {
    ...
    this.msgText = this.add.text(w / 2, h / 2 - 15, '', { ...mono, fontSize: '24px', color: '#ffffff', align: 'center' }).setOrigin(0.5).setDepth(25000).setScrollFactor(0).setAlpha(0);
    this.subMsgText = this.add.text(w / 2, h / 2 + 18, '', { ...mono, fontSize: '13px', color: '#7080aa', align: 'center' }).setOrigin(0.5).setDepth(25000).setScrollFactor(0).setAlpha(0);
    ...
    this.keys = {
      ...
      e: this.input.keyboard!.addKey('E'), m: this.input.keyboard!.addKey('M'),
  ...
  // ===== Zones & Layers =====
  private get zone() { return Math.ceil(this.layer / 6); }
  private get subLayer() { return ((this.layer - 1) % 6) + 1; }
  private get isBossLayer() { return this.subLayer === 6; }

  private getArenaBounds() {
    const w = this.scale.width, h = this.scale.height, ci = this.collapseInset;
    const top = 22, bot = 28, side = 8;
    return { x: ci + side, y: top + ci, w: w - ci * 2 - side * 2, h: h - top - bot - ci * 2 };
  }
  ...
  private beginLayer() {
    ...
    if (this.isBossLayer) { this.startBoss(); return; }

    this.weaponTier = Math.min(5, Math.floor(this.layer / 7) + 1);
  ...
  private buildSpawnQueue(total: number) {
    ...
    for (let i = 0; i < total; i++) {
      ...
      const delay = batch * 800 + Math.random() * 400;
  ...
  private spawnOneEnemy(type: EnemyType) {
    ...
    switch (edge) {
      case 0: ex = bounds.x + 20 + Math.random() * (bounds.w - 40); ey = bounds.y + 20; break;
      case 1: ex = bounds.x + 20 + Math.random() * (bounds.w - 40); ey = bounds.y + bounds.h - 20; break;
      case 2: ex = bounds.x + 20; ey = bounds.y + 20 + Math.random() * (bounds.h - 40); break;
      default: ex = bounds.x + bounds.w - 20; ey = bounds.y + 20 + Math.random() * (bounds.h - 40); break;
    }
  ...
  private advanceLayer() {
    ...
    if (this.layer > 42) { this.triggerVictory(); return; }
  ...
  private getLayerUnlock(): string | null {
    const unlocks: Record<number, string> = {
      3: 'NEW ENEMY: clickbait, explosive kamikaze',
      5: 'NEW WEAPON: piercing rounds unlocked',
      7: 'NEW ENEMY: bias, lunging predator',
      10: 'NEW WEAPON: homing shots unlocked',
      13: 'NEW ENEMY: deepfake, shapeshifter',
      16: 'NEW WEAPON: chain lightning unlocked',
      19: 'NEW ENEMY: scraper, marching box',
      25: 'NEW ENEMY: overfit, fractal hunter',
      30: 'WEAPON TIER 5, max power',
    };
  ...
  private showMessage(text: string, sub: string, duration: number) {
    ...
    this.tweens.add({ targets: this.msgText, alpha: 0, delay: duration - 500, duration: 500 });
    this.tweens.add({ targets: this.subMsgText, alpha: 0, delay: duration - 500, duration: 500 });
  }
  ...
  private updateHUD() {
    this.hud.updateValues(this.contextLevel, this.player.health, this.player.maxHealth, this.player.tokens, this.player.promptCharges, this.player.maxPromptCharges, this.player.currentPromptCd, this.player.promptCooldown, this.layer, this.combo, this.collapseActive, this.player.weaponMod, this.player.weaponModTimer);
    this.hud.draw();
  }
  ...
  private updateInput(delta: number) {
    ...
    if (this.boss && !this.boss.isDead && this.boss.piGlitchActive) { mx = -mx; my = -my; }

    this.player.update(delta, mx, my, this.getArenaBounds());

    const nearest = this.findNearest(350);
    if (nearest) { this.player.aimX = nearest.x; this.player.aimY = nearest.y; }
    else { this.player.aimX = this.player.x + this.player.lastMoveX * 200; this.player.aimY = this.player.y + this.player.lastMoveY * 200; }

    const enemyPositions = this.enemies.filter(e => !e.isDead).map(e => ({ x: e.x, y: e.y }));
    if (this.keys.space.isDown) {
      if (this.player.shoot(this.projectiles, enemyPositions)) this.contextLevel += 0.12;
    }
  ...
  private useSpecialAbility() {
    if (this.player.promptCharges <= 0 || this.player.currentPromptCd > 0) return;
    ...
    switch (this.player.systemPrompt) {
      ...
      case 'hallucinate':
        ...
        for (const e of this.enemies) {
          ...
          e.speed *= 0.1;
          this.time.delayedCall(2000, () => { if (!e.isDead) e.speed = e.cfg.speed; });
        }
        break;
      case 'analyze':
        ...
        for (const e of this.enemies) {
          if (e.isDead) continue;
          e.speed *= 0.5;
          this.time.delayedCall(5000, () => { if (!e.isDead) e.speed = e.cfg.speed; });
        }
        break;
      case 'jailbreak':
  ...
  private deployTurret() {
    ...
    const turretTimer = this.time.addEvent({
      delay: 16, loop: true,
      callback: () => {
        turretLife -= 16; turretCd -= 16; gfx.clear();
        if (turretLife <= 0) { gfx.destroy(); turretTimer.destroy(); return; }
        const a = turretLife / 8000;
        gfx.fillStyle(0x00aaff, 0.15 * a); gfx.fillCircle(tx, ty, 20);
        gfx.lineStyle(1, 0x00aaff, 0.5 * a); gfx.strokeRect(tx - 8, ty - 8, 16, 16);
        gfx.fillStyle(0x00aaff, 0.7 * a); gfx.fillRect(tx - 3, ty - 3, 6, 6);
        if (turretCd <= 0) {
          const near = this.findNearest(300);
          if (near) {
            turretCd = 350;
            const angle = Math.atan2(near.y - ty, near.x - tx);
            this.projectiles.push(new Projectile(this, tx, ty, Math.cos(angle) * 400, Math.sin(angle) * 400, 10, 0x00aaff, 1000, true));
          }
        }
      },
    });
  ...
  private updateSpawnQueue(delta: number) {
    ...
    while (this.spawnQueue.length > 0 && this.spawnQueue[0].delay <= this.spawnAccum) {
  ...
  private updateProjectiles(delta: number) {
    ...
    for (let i = this.projectiles.length - 1; i >= 0; i--) {
      ...
      if (!p.scene) { this.projectiles.splice(i, 1); continue; }
      if (!p.update(delta)) { this.projectiles.splice(i, 1); continue; }
      if (p.x < bounds.x || p.x > bounds.x + bounds.w || p.y < bounds.y || p.y > bounds.y + bounds.h) { p.destroy(); this.projectiles.splice(i, 1); }
    }
  }
  ...
  private updateEnemies(delta: number) {
    for (let i = this.enemies.length - 1; i >= 0; i--) {
      ...
      if (!e.scene) { this.enemies.splice(i, 1); continue; }
      const result = e.update(delta, this.player.x, this.player.y);
      if (result.dead) { this.enemies.splice(i, 1); continue; }
      if (result.attack && !e.isDead) {
        const dist = Phaser.Math.Distance.Between(e.x, e.y, this.player.x, this.player.y);
        if (dist < e.radius + this.player.radius + 10) this.player.takeDamage(e.damage);
        if (e.enemyType === 'clickbait') { e.takeDamage(e.maxHealth); this.spawnExplosion(e.x, e.y, 0xff5500); }
      }
      const bounds = this.getArenaBounds();
  ...
  private updateBoss(delta: number) {
    ...
    if (this.boss.wantsSpawn && this.enemies.length < 16) {
      const types = ZONE_ENEMIES[Math.min(this.zone - 1, ZONE_ENEMIES.length - 1)];
      const type = types[Math.floor(Math.random() * types.length)];
      this.enemies.push(new Enemy(this, this.boss.spawnX, this.boss.spawnY, type, this.zone));
    }
    ...
    if (this.boss.isDead && this.boss.deathCinematic) {
      if (this.gameState !== 'bossDeath') {
        ...
        this.time.delayedCall(2500, () => {
          this.boss = null;
          if (this.layer >= 42) { this.triggerVictory(); }
          else { this.advanceLayer(); }
        });
      }
    }
  ...
  private updatePickups(delta: number) {
    for (let i = this.pickups.length - 1; i >= 0; i--) {
      ...
      p.age += delta;
      if (p.age > 10000 || p.collected) { this.pickups.splice(i, 1); continue; }
      const dist = Phaser.Math.Distance.Between(p.x, p.y, this.player.x, this.player.y);
      if (dist < 24) {
        p.collected = true;
        if (p.type === 'token') { this.player.tokens++; audio.play('tokenCollect'); }
        else if (p.type === 'health') { this.player.heal(15 + this.zone * 5); }
          ...
          this.showMessage(WEAPON_MOD_NAMES[p.weaponMod] + tierLabel, `${Math.round(dur / 1000)}s`, 1000);
        }
      }
    }
  }
  ...
  private updateContext(delta: number) {
    const rate = this.gameState === 'boss' ? 1.8 + this.zone * 0.15 : 1.0 + this.zone * 0.12;
    this.contextLevel += delta / 1000 * rate;
    this.contextLevel = Math.min(150, this.contextLevel);
    if (this.contextLevel >= 100 && !this.collapseActive) { this.collapseActive = true; audio.play('contextCollapse'); }
  }

  private updateCollapse(delta: number) {
    if (!this.collapseActive) { this.collapseInset = Math.max(0, this.collapseInset - delta * 0.12); return; }
    this.collapseInset += delta * 0.022;
    const bounds = this.getArenaBounds();
    if (bounds.w < 100 || bounds.h < 80) { this.player.takeDamage(this.player.maxHealth); return; }
    const margin = 15;
    if (this.player.x < bounds.x + margin || this.player.x > bounds.x + bounds.w - margin || this.player.y < bounds.y + margin || this.player.y > bounds.y + bounds.h - margin) {
  ...
  private checkCollisions() {
    for (let pi = this.projectiles.length - 1; pi >= 0; pi--) {
      ...
      for (const enemy of this.enemies) {
        ...
        if (Phaser.Math.Distance.Between(proj.x, proj.y, enemy.x, enemy.y) < proj.radius + enemy.radius) {
          proj.hitIds.add(enemy.eid);
          let dmg = proj.damage;
          if (this.player.systemPrompt === 'analyze' && enemy.health / enemy.maxHealth < 0.4) dmg *= 1.25;
          ...
          if (proj.chainBounces > 0 && killed) {
            const chainTarget = this.enemies.find(e => !e.isDead && e.eid !== enemy.eid && Phaser.Math.Distance.Between(enemy.x, enemy.y, e.x, e.y) < proj.chainRange);
            if (chainTarget) {
          ...
          if (!proj.piercing) { proj.destroy(); this.projectiles.splice(pi, 1); break; }
        }
      }
    }
    for (const e of this.enemies) {
      ...
      if (d < e.radius + this.player.radius) {
        ...
        this.player.x += Math.cos(a) * 4; this.player.y += Math.sin(a) * 4;
        this.combo = 0;
      }
    }
  }
  ...
  private spawnPickup(x: number, y: number, isWeaponDrop: boolean) {
    const pool = ZONE_WEAPON_POOL[Math.min(this.zone - 1, ZONE_WEAPON_POOL.length - 1)];
    if (isWeaponDrop) {
      const mod = pool[Math.floor(Math.random() * pool.length)];
      this.pickups.push({ x, y, type: 'weapon', weaponMod: mod, tier: this.weaponTier, age: 0, collected: false });
    }
    const tokenChance = this.player.systemPrompt === 'hallucinate' ? 0.78 : 0.62;
    const comboBonus = this.combo >= 5 ? 0.15 : 0;
    if (Math.random() < tokenChance + comboBonus) this.pickups.push({ x: x + (Math.random() - 0.5) * 14, y: y + (Math.random() - 0.5) * 14, type: 'token', age: 0, collected: false });
    if (Math.random() < 0.12 + (this.combo >= 10 ? 0.08 : 0)) this.pickups.push({ x: x + (Math.random() - 0.5) * 14, y: y + (Math.random() - 0.5) * 14, type: 'health', age: 0, collected: false });
  }

  private spawnExplosion(x: number, y: number, color: number) {
    const gfx = this.add.graphics().setDepth(9500);
    let radius = 5;
    const timer = this.time.addEvent({ delay: 16, repeat: 15, callback: () => { radius += 4; gfx.clear(); gfx.lineStyle(2, color, 1 - timer.getProgress()); gfx.strokeCircle(x, y, radius); } });
    this.time.delayedCall(300, () => gfx.destroy());
    for (const e of this.enemies) { if (!e.isDead && Phaser.Math.Distance.Between(x, y, e.x, e.y) < 60) e.takeDamage(10); }
    if (Phaser.Math.Distance.Between(x, y, this.player.x, this.player.y) < 60) this.player.takeDamage(12);
  }
  ...
  private checkLayerProgress() {
    ...
    const allSpawned = this.layerEnemiesSpawned >= this.layerEnemiesTotal && this.spawnQueue.length === 0;
  ...
  private checkPlayerDeath() {
    if (this.player.isDead && this.gameState !== 'gameOver') {
      ...
      audio.play('gameOver');
      this.time.delayedCall(1200, () => this.scene.start('GameOverScene', { layer: this.layer, kills: this.totalKills, tokens: this.player.tokens }));
    }
  }
  ...
  private triggerVictory() {
    ...
    audio.play('victory');
    this.time.delayedCall(1500, () => this.scene.start('VictoryScene', { tokens: this.player.tokens, systemPrompt: this.systemPrompt, layers: this.layer }));
  }
  ...
  private drawArena() {
    const w = this.scale.width, h = this.scale.height;
    this.arenaGfx.clear();
    this.arenaGfx.lineStyle(1, 0x3f4466, 0.18);
    ...
    if (this.collapseActive) {
      ...
      this.arenaGfx.lineStyle(1, 0xff2222, ba);
  ...
  private drawCollapse() {
    ...
    this.collapseGfx.fillStyle(0xff2222, a * 0.35);
    ...
    this.collapseGfx.fillRect(w - ci - side, top + ci, ci + side, h - top - 28 - ci * 2);
    this.collapseGfx.lineStyle(1, 0xff2222, a * 0.5);
    for (let i = 0; i < 4; i++) { const y = top + Math.random() * ci; this.collapseGfx.lineBetween(Math.random() * w, y, Math.random() * w * 0.3 + Math.random() * w * 0.7, y); }
  }
  ...
  private drawCombo() {
    ...
    const c = this.combo >= 10 ? 0xffcc00 : this.combo >= 5 ? 0x00ddff : 0x7788aa;
  ...
  private drawPickups() {
    ...
    for (const p of this.pickups) {
      ...
      if (p.type === 'token') {
        this.pickupGfx.fillStyle(0xffcc00, 0.9 * fade);
        this.pickupGfx.beginPath(); this.pickupGfx.moveTo(p.x, p.y - 5 + bob); this.pickupGfx.lineTo(p.x + 5, p.y + bob); this.pickupGfx.lineTo(p.x, p.y + 5 + bob); this.pickupGfx.lineTo(p.x - 5, p.y + bob); this.pickupGfx.closePath(); this.pickupGfx.fillPath();
      } else if (p.type === 'health') {
        this.pickupGfx.fillStyle(0x00e89a, 0.9 * fade);
        this.pickupGfx.fillRect(p.x - 2, p.y - 5 + bob, 4, 10); this.pickupGfx.fillRect(p.x - 5, p.y - 2 + bob, 10, 4);
      } else if (p.type === 'weapon' && p.weaponMod) {
        const wc = WEAPON_MOD_COLORS[p.weaponMod];
        const tier = p.tier ?? 1;
        const glowR = 8 + tier * 2;
        this.pickupGfx.fillStyle(wc, 0.15 * fade); this.pickupGfx.fillCircle(p.x, p.y + bob, glowR);
        this.pickupGfx.fillStyle(wc, 0.85 * fade);
        this.pickupGfx.beginPath(); this.pickupGfx.moveTo(p.x, p.y - 6 + bob); this.pickupGfx.lineTo(p.x + 6, p.y + bob); this.pickupGfx.lineTo(p.x, p.y + 6 + bob); this.pickupGfx.lineTo(p.x - 6, p.y + bob); this.pickupGfx.closePath(); this.pickupGfx.fillPath();
        this.pickupGfx.lineStyle(1, wc, 0.5 * fade); this.pickupGfx.strokeCircle(p.x, p.y + bob, glowR);
        if (tier >= 3) { this.pickupGfx.lineStyle(1, 0xffffff, 0.15 * fade); this.pickupGfx.strokeCircle(p.x, p.y + bob, glowR + 3); }
      }
    }
  }
}
