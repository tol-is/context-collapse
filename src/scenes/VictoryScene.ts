...
export default class VictoryScene extends Phaser.Scene {
  ...
  create(data: { tokens?: number; systemPrompt?: string; layers?: number }) {
    ...
    const lines = [
      { text: '$ run --stabilize', color: '#445566', delay: 0 },
      { text: '', color: '#445566', delay: 300 },
      { text: 'CONTEXT STABILIZED', color: '#00cc66', delay: 600 },
      { text: 'The singularity has been contained.', color: '#667788', delay: 1000 },
      { text: '', color: '#445566', delay: 1200 },
      { text: `  system_prompt   "${data.systemPrompt ?? 'autocomplete'}"`, color: '#00bbdd', delay: 1600 },
      { text: `  layers_cleared  ${data.layers ?? 42}`, color: '#00bbdd', delay: 2000 },
      { text: `  tokens          ${data.tokens ?? 0}`, color: '#00bbdd', delay: 2400 },
      { text: '  status          OUTPUT_CLEAN', color: '#00cc66', delay: 2800 },
      { text: '', color: '#445566', delay: 3200 },
      { text: '> The model generates clearly now.', color: '#667788', delay: 3600 },
      { text: '> For a little while, at least.', color: '#667788', delay: 4200 },
    ];
  ...
  update(_time: number, delta: number) {
    ...
    if (this.blinkTimer > 500) {
      ...
      if (this.promptReady) {
        this.promptText.setText('> PRESS ENTER TO REPLAY' + (this.cursorBlink ? '_' : ' '));
      }
    }
